...
# Clean the data - remove any non-numeric columns or convert them
print("\nCleaning data...")
...
if non_numeric_columns:
    ...
    print("These columns will be excluded from analysis")

# Use only numeric columns
df_clean = df[numeric_columns + ['target_binary']].copy()
...
for col in df_clean.columns:
    if col == 'target_binary':
        continue
    elif df_clean[col].nunique() <= 2 and set(df_clean[col].unique()).issubset({0, 1, 0.0, 1.0}):
...
# Get correlation results to identify top features
try:
    ...
    # Filter to only include features that exist in our cleaned dataset
    top_features = [f for f in top_features if f in df_engineered.columns]
except:
    # If correlation results not available, use numeric features
...
available_continuous = [var for var in continuous_vars if var in df_engineered.columns]
...
X = df_engineered.drop(['target_binary'], axis=1)
y = df_engineered['target_binary']

print(f"Features before selection: {X.shape[1]}")

# 2.1 Remove any remaining non-finite values
...
print(f"Top features by univariate selection: {len(selected_features_univariate)}")
...
top_mi_features = mi_results.head(min(30, len(mi_results)))['feature'].tolist()
...
rfe = RFE(estimator=rfe_model, n_features_to_select=min(30, X.shape[1]), step=1)
...
top_rf_features = rf_importance.head(min(30, len(rf_importance)))['feature'].tolist()

# 2.6 Consensus feature selection
...
print(f"Engineered features: {df_engineered.shape[1] - df_clean.shape[1]}")
print(f"Final selected features: {len(final_features)}")
