...
# Create comprehensive visualization suite - PROPER SUBPLOT CREATION
...
for i, feature in enumerate(numeric_top):
    ...
    box_plot = axes[row, col].boxplot(box_data, labels=['Reject', 'Hire'], patch_artist=True)
    ...
    axes[row, col].text(0.02, 0.98, f'p={p_val:.3f} {significance}', transform=axes[row, col].transAxes, 
...
for i, feature in enumerate(numeric_top):
    ...
    axes[row, col].hist(reject_data, bins=20, alpha=0.6, color='lightcoral', label='Reject', density=True)
    axes[row, col].hist(hire_data, bins=20, alpha=0.6, color='lightgreen', label='Hire', density=True)
...
for i, feature in enumerate(binary_top):
    row, col = (i + 12) // 4, (i + 12) % 4
    
    # Calculate proportions
    contingency = pd.crosstab(df[feature], df['target_binary'], normalize='index')
    
    contingency.plot(kind='bar', ax=axes[row, col], color=['lightcoral', 'lightgreen'], alpha=0.7)
    axes[row, col].set_title(f'{feature}', fontsize=10, fontweight='bold')
    axes[row, col].legend(['Reject', 'Hire'])
    axes[row, col].set_xlabel('Feature Value')
    axes[row, col].set_ylabel('Proportion')
    axes[row, col].grid(True, alpha=0.3)
    plt.setp(axes[row, col].get_xticklabels(), rotation=0)

# 4. Binary features - Stacked bar plots (positions 19-24)
print("Creating binary features stacked bar plots...")
for i, feature in enumerate(binary_top):
    row, col = (i + 18) // 4, (i + 18) % 4
    
    # Calculate counts
    contingency_counts = pd.crosstab(df[feature], df['target_binary'])
    
    contingency_counts.plot(kind='bar', stacked=True, ax=axes[row, col], 
                             color=['lightcoral', 'lightgreen'], alpha=0.7)
    axes[row, col].set_title(f'{feature} Counts', fontsize=10, fontweight='bold')
    axes[row, col].legend(['Reject', 'Hire'])
    axes[row, col].set_xlabel('Feature Value')
