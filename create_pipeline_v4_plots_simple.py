...
def create_comprehensive_plots():
    ...
    ax1.bar(x - width/2, original_jobs, width, label='Original', alpha=0.8, color='lightcoral')
    ax1.bar(x + width/2, balanced_jobs, width, label='Balanced', alpha=0.8, color='lightgreen')
    ax1.set_xlabel('Job Types')
    ax1.set_ylabel('Proportion')
    ax1.set_title('Job Type Distribution: Original vs Balanced')
    ax1.set_xticks(x)
    ax1.set_xticklabels([col.replace('feat_job_', '').title() for col in job_cols], rotation=45)
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Pie chart for balanced dataset
    ax2.pie(balanced_jobs, labels=[col.replace('feat_job_', '').title() for col in job_cols], 
    ...
    ax1.bar(x - width/2, original_skills, width, label='Original', alpha=0.8, color='lightcoral')
    ax1.bar(x + width/2, balanced_skills, width, label='Balanced', alpha=0.8, color='lightgreen')
    ...
    # Skills correlation heatmap (simplified)
    skills_matrix = balanced_df[skill_cols].corr()
    im = ax2.imshow(skills_matrix, cmap='coolwarm', aspect='auto')
    ax2.set_xticks(range(len(skill_cols)))
    ax2.set_yticks(range(len(skill_cols)))
    ax2.set_xticklabels([col.replace('feat_skill_', '').title() for col in skill_cols])
    ax2.set_yticklabels([col.replace('feat_skill_', '').title() for col in skill_cols])
    ax2.set_title('Skills Correlation Matrix (Balanced)')
    
    # Add correlation values
    for i in range(len(skill_cols)):
        for j in range(len(skill_cols)):
            ax2.text(j, i, f'{skills_matrix.iloc[i, j]:.2f}', 
                    ha='center', va='center', color='white' if abs(skills_matrix.iloc[i, j]) > 0.5 else 'black')
    ...
    ax1.bar(x - width/2, original_all, width, label='Original', alpha=0.8, color='lightcoral')
    ax1.bar(x + width/2, balanced_all, width, label='Balanced', alpha=0.8, color='lightgreen')
    ...
    ax1.bar(x - width/2, original_certs, width, label='Original', alpha=0.8, color='lightcoral')
    ax1.bar(x + width/2, balanced_certs, width, label='Balanced', alpha=0.8, color='lightgreen')
    ...
    for cert_col in cert_cols:
        ...
        cert_summary.append(coverage)
        cert_names.append(cert_col.replace('feat_cert_', '').upper())
    
    colors = ['green' if x >= 0.04 else 'red' for x in cert_summary]
    ax2.barh(cert_names, cert_summary, color=colors, alpha=0.8)
    ax2.set_xlabel('Coverage Proportion')
    ax2.set_title('Certification Coverage (Balanced Dataset)')
    ax2.axvline(x=0.04, color='red', linestyle='--', alpha=0.7, 
               label='Minimum 4% Target')
    ax2.legend()
    ...
    numeric_cols = ['feat_num_skills_matched', 'feat_years_experience_extracted', 'feat_salary_extracted']
    
    for i, col in enumerate(numeric_cols):
        if col == 'feat_salary_extracted':
            ax3.hist(balanced_df[col], bins=30, alpha=0.7, color='blue', 
                    label='Salary Distribution')
            ax3.set_xlabel('Salary ($)')
            ax3.set_ylabel('Frequency')
            ax3.set_title('Salary Distribution (Balanced Dataset)')
            ax3.grid(True, alpha=0.3)
            break
    
    # Skills count distribution
    ax4.hist(balanced_df['feat_num_skills_matched'], bins=range(0, 6), alpha=0.7, color='purple')
    ax4.set_xlabel('Number of Skills Matched')
    ax4.set_ylabel('Frequency')
    ax4.set_title('Skills Count Distribution (Balanced Dataset)')
